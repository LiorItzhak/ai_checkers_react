        ...
        try {
            ...
            while (maxIterations?.let { rootNode.numOfVisits - n < it } != false) {
                ...
                if (rootNode.numOfVisits % 150 == 0) {
                    delay(1);console.info("mcts weight= ${chosenNode!!.estimate()} | ${chosenNode!!.numOfVisits}/${rootNode.numOfVisits}")
                }
            }
        } finally {
            statesCache?.clear()
            ...
            console.info("finally: weight= ${chosenNode?.estimate()} | ${chosenNode!!.numOfVisits}/${rootNode.numOfVisits} | visits from previous search= $n,depth=$d |used cached state =$numOfUsedCachedStates")
