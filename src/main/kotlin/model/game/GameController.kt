...
class GameController<T : BoardGame<M, B>, B : Board<out Piece>, M : Move>(
    ...
    private fun endGame() {
        ...
        val winner = if (player1Score > player2Score) player2.player else if (player1Score < player2Score) player2.player else null
    ...
    private suspend fun idle() {
        while (!game.isGameEnded(turn)) {
            ...
            val move: M? = when (timeLimitMillis) {
                null -> currentPlayer.startTurn(game)
                else -> withTimeoutOrNull(timeLimitMillis) { currentPlayer.startTurn(game) }
            }
            ...
            if (!game.getAllPossibleMoves(turn).any { it == move })
                throw RuntimeException("${currentPlayer.player} played illegal move :$move")
